...
const ShweayButton = ({ label = 'Contact', className = '', ...buttonProps }: ShweayButtonProps) => {
  return (
    <button className={`relative text-center  w-full h-full z-40 ${className}`} {...buttonProps}>
      ...
      <div className="absolute inset-1.5 xl:inset-3 bg-linear-to-b from-grad-four to-grad-five rounded-sm xl:rounded-lg"></div>
      <div className="absolute inset-1.5 xl:inset-3 bg-grad-five opacity-50 rounded-sm xl:rounded-lg"></div>
      <div className="absolute inset-1.5 xl:inset-3 rounded-sm xl:rounded-lg border-[2px] border-transparent bg-gradient-to-r from-grad-five/50 to-grad-five bg-clip-border"></div>
      <div className="absolute inset-1.5 sm:inset-3 rounded-sm sm:rounded-lg flex items-center justify-center">
        <span className="uppercase text-black font-bold text-[14px] xl:text-[1.5em]">{label}</span>
      </div>
    </button>
  )
}
